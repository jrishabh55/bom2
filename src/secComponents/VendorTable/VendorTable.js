...
export class VendorTable extends Component {
    ...
    componentDidMount() {
        if (this.vendorId !== 'new') {
            ApiService.get(`/customer/${VendorContract.getCustomerId}/bom/${this.vendorId}`).then(res => {
                this.setState({currData: res.estimate.line_items},() => console.log(this.state.currData))
    ...
    quotedData($index) {
        let data = {};
        data.list_price = $(`#list_price${$index}`)[0].valueAsNumber;
        data.discount = $(`#discount${$index}`)[0].valueAsNumber;
        data.time_to_ship = $(`#timeToShip${$index}`)[0].valueAsNumber;
        data.current_stock = $(`#stock${$index}`)[0].valueAsNumber;
        data.HSN = $(`#hsn${$index}`)[0].value;
        data.GST = $(`#gst${$index}`)[0].valueAsNumber;
        data.delivery_city = $(`#delivery_city${$index}`)[0].value;
        data.remarks = $(`#remarks${$index}`)[0].value;
        return data;
    }
    ...
    makeQuote() {
        this.state.currData.map(($data, $index) => {
            ...
            const data =  this.quotedData($index);
                    ...
                    <Table bordered>
                            ...
                            <tbody>
                            {
                                this.state.currData.map(($data, $index) => {
                                    ...
                                    <td>{$data.quantity || '-'}</td>
                                    <td><Input id={`stock${$index}`} type="number" disabled={this.state.editable ? null : 'disabled'} defaultValue={'-'} /></td>
                                    <td><Input id={`gst${$index}`} type="number" disabled={this.state.editable ? null : 'disabled'} defaultValue={$data.tax_percentage||'-'} /></td>
                                    <td><Input id={`hsn${$index}`} type="text" disabled={this.state.editable ? null : 'disabled'} defaultValue={'--'} /></td>
                                    <td><Input type="text" disabled={this.state.editable ? null : 'disabled'} defaultValue={$data.item_total || '-'} /></td>
                                    <td><Input id={`discount${$index}`} type="number" disabled={this.state.editable ? null : 'disabled'} defaultValue={$data.discount||'-'} /></td>
                                    <td><Input id={`timeToShip${$index}`} type="number" disabled={this.state.editable ? null : 'disabled'} defaultValue={'-'} /></td>
                                    <td><Input id={`delivery_city${$index}`} type="text" disabled={this.state.editable ? null : 'disabled'} defaultValue={'--'} /></td>
                                    <td><Input id={`list_price${$index}`} type="number" disabled={this.state.editable ? null : 'disabled'} defaultValue={'--'} /></td>
                                    <td>{'-'}</td>
                                    <td>{'-'}</td>
                                    <td><Input id={`remarks${$index}`} type="text" disabled={this.state.editable ? null : 'disabled'} defaultValue={'-'} /></td>
